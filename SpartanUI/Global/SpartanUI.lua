...
---------------------------------------------------------------------------------------------------
local CurrentVersion = addon.CurrentVersion;
...
---------------------------------------------------------------------------------------------------
function Sui_OnInitialize()
	SlashCmdList["SUI"] = Sui_SlashCommand -- Sui.SlashCommand
	SLASH_SUI1 = "/sui"
	SLASH_SUI2 = "/spartanui"

	addon.delayedCalls = {}
	addon.cameraDistanceMax = GetCVar("cameraDistanceMax")
	
	for __,v in pairs(EventList) do SUI_HUD:RegisterEvent(v) end
end
-----------------------------------------------------------------------------------------------
...
function Sui_Setup()
	...
	ClickCastFrames[SUI_TOT_Button] = true
end
-----------------------------------------------------------------------------------------------
...
function Sui_SlashCommand(msg)
	...
	if msg=="resetbars" then
		suiChar.BT4Profile = nil
		if addon.BartenderProfile then
			addon:BartenderProfile()
		end
	elseif msg=="backdrops" then
		for i = 1,6 do -- for each BT4 Bar Backdrop
			if (suiChar["bar"..i]) and (suiChar["bar"..i] == 1) then
				suiChar["bar"..i] = 0;
			else
				suiChar["bar"..i] = 1;
			end
		...
		for i = 1,2 do -- for each BT4 Popup Bar
			if (suiChar["popup"..i]) and (suiChar["popup"..i] == 1) then
				suiChar["popup"..i] = 0;
			else
				suiChar["popup"..i] = 1;
			end
	...
	elseif msg==SUIL.ENGLISH.UI.MAXRES or msg==SUI_Lang.UI.MAXRES then
		if (suiData.relScale == 0.92) then
			suiData.relScale = 0.78;
		else
			suiData.relScale = 0.92;
		end
	elseif msg==SUIL.ENGLISH.UI.VERSION or msg==SUI_Lang.UI.VERSION or msg=="vers" then
		local channel
		if arg1 then channel = "WHISPER"
		elseif UnitInRaid("player")then channel = "RAID"
		elseif GetNumPartyMembers() > 0 then channel = "PARTY"
		elseif GetGuildInfo("player") then channel = "GUILD"
		else channel = nil end
		if channel then
...
function Sui_BuffTooltip_Enter()
	...
	while frame do
    	if frame:IsVisible() and MouseIsOver(frame) then
			...
			if start then -- if a match is found, setup tooltip and show
				...
				if (state=="Buff") then
					...
					GameTooltip:SetUnitAura(toon, index, "HELPFUL")
					GameTooltip:Show()
				elseif (state=="Debuff") then
					GameTooltip:SetOwner(father, direction)
					GameTooltip:SetUnitAura(toon, index, "HARMFUL")
					GameTooltip:Show()
				elseif (state=="Weapon") and (toon=="player") then
					GameTooltip:SetOwner(father, direction)
					GameTooltip:SetInventoryItem(toon,index+15)
					GameTooltip:Show()
				end
    	    	suiData.tooltipUpdate = true -- toggle for repeat updates of tooltips
    	    end
    	end
...
function Sui_DelayedCall(functionName, parameters, timeDelay)
	if not functionName then
		if addon.delayedCalls and addon.delayedCalls[1] then
			local i = 1
			while i and i<5 do
				if addon.delayedCalls[i] and addon.delayedCalls[i][3] <= GetTime() then
					_G[addon.delayedCalls[i][1]](unpack(addon.delayedCalls[i][2]))
					-- DEFAULT_CHAT_FRAME:AddMessage(addon.delayedCalls[i][1].." called.", 0.5, 0.5, 0.5)
					table.remove(addon.delayedCalls, i)
				elseif addon.delayedCalls[(i+1)] then
					i = i + 1
				else
					i = nil
				end
		...
		if not addon.delayedCalls then
			addon.delayedCalls = {}
		end
		table.insert(addon.delayedCalls, i)
	end
end
-----------------------------------------------------------------------------------------------
...
function Sui_SpinCam()
	...
	if (toggle == "1") then
		if (SpinCam == true) then
			MoveViewRightStop()
			SetCVar("cameraYawMoveSpeed","230")
			SetCVar("cameraDistanceMax",addon.cameraDistanceMax)
		else
...
function Sui_OnEvent()
		...
		if arg1=="SUI" then
			...
			if task=="find" then
				...
				elseif subject=="version" then
					SendAddonMessage("SUI", "deliver "..UnitName("player").." "..subject.." "..addon.CurrentVersion, "WHISPER", caller)
	...
	elseif event=="CHAT_MSG_SYSTEM" then
		local afkmsg = format(MARKED_AFK_MESSAGE,DEFAULT_AFK_MESSAGE)
		if ( arg1 == afkmsg ) then -- this should work in all locales
			SpinCam = false
		...
		elseif ( arg1 == CLEARED_AFK) then -- this should work in all locales
			SpinCam = true
	...
	elseif event=="UNIT_HEALTH" or event=="UNIT_MANA" or event=="UNIT_RAGE" or event=="UNIT_ENERGY" or event=="UNIT_RUNIC_POWER" then
	...
	elseif event=="UNIT_PORTRAIT_UPDATE" then
		...
		if arg1=="target" and UnitExists("targettarget") then
	...
	elseif event=="PLAYER_ENTERING_WORLD" then
		if RuneFrame then
			RuneFrame:SetParent(SpartanUI);
			RuneFrame:SetFrameStrata("HIGH");
			RuneFrame:ClearAllPoints()
			RuneFrame:SetPoint("TOPRIGHT", SpartanUI, "TOP", -160, -100)
		end
		Sui_SetupMembers("all", "setup")
		Sui_Unit_Update()
...
function SUI_PanelOffSet()
	...
	SpartanUI:SetPoint("BOTTOM",UIParent,"BOTTOM",0,offset);
	...
	if suiData then
		...
		if (FastInterval > (UpdateInterval/32)) then				-- 32 FPS
			if SUI_MouseTooltip:IsVisible() then
				...
				local realScale = (UIParent:GetEffectiveScale())*(suiData.relScale)
