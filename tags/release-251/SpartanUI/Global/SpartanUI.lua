local CurrentVersion = 20081018; -- 2.5.1
local Sui = LibStub:NewLibrary("SpartanUI",CurrentVersion)
...
---------------------------------------------------------------------------------------------------
local DebugEnabled, GodMode, SpinCam = true, false, false;
local FrameAdvance = 0;
local UpdateInterval = 1;
local GeneralInterval = 0;
local AnimationInterval = 0;
local FastInterval = 0;
local FrequentInterval = 0;
local classIcons = {
	WARRIOR = {1,1},
	MAGE = {1,2},
	ROGUE = {1,3},
...
if (not suiData.version) or (suiData.version < CurrentVersion) then
	-- variables that need to be kept between updates
	local defaults = {
		buffToggle = true,
		partyToggle = "on",
		scale = 0.78,
	...
	for k,v in pairs(defaults) do suiData[k] = suiData[k] or v; end
	suiData.version = CurrentVersion
end
if (type(suiChar)~="table") then suiChar = {} end
---------------------------------------------------------------------------------------------------

function Sui_OnInitialize() -- automatically executed at ADDON_LOADED
	SlashCmdList["SUI"] = Sui_SlashCommand -- Sui.SlashCommand
	SLASH_SUI1 = "/sui"
	SLASH_SUI2 = "/spartanui"

	Sui.SessionXP = UnitXP("player")
	Sui.SessionTime = GetTime()
	Sui.delayedCalls = {}

	for __,v in pairs(EventList) do SpartanUI:RegisterEvent(v) end
end
-----------------------------------------------------------------------------------------------
...
function Sui_SlashCommand(msg)
	...
	if msg=="resetbars" then
		suiChar.BT4 = nil
		Sui_Bartender()
	elseif msg=="faction" then
		if not suiData.autoFaction then
			suiData.autoFaction = true
		else
			suiData.autoFaction = nil
		end
	elseif msg==SUIL.ENGLISH.UI.GODMODE or msg==SUI_Lang.UI.GODMODE then
		local enemy
		if UnitFactionGroup("player")=="Alliance" then enemy="Horde"
		elseif UnitFactionGroup("player")=="Horde" then enemy="Alliance"
		else enemy="poor innocent (and horrible cute) bunnies" end
		DEFAULT_CHAT_FRAME:AddMessage("Entering God Mode.", 1.0, 0, 0.0)
		DEFAULT_CHAT_FRAME:AddMessage("Now go beat up some "..enemy..", "..UnitName("player")..".  :)", .5, .5, .5)
		GodMode=true
	...
	elseif msg=="lang" then
		if arg1=="english" or arg1==SUIL.ENGLISH.LANGUAGE then
			...
			DEFAULT_CHAT_FRAME:AddMessage("SpartanUI v"..suiData.version.." "..SUI_Lang.UI.LANGUAGE.." loaded.", 0.65, 0.65, 0.65)
...
function Sui_Bartender()
	...
	if not suiChar.BT4 then
		...
		Bartender4.db:SetProfile(userprofile)
		Bartender4.db:CopyProfile(default)
	...
	if BT4Bar6 then
		...
		SUI_Bar5_BG:Show()
		SUI_Bar5_BG:SetPoint("BOTTOMRIGHT", "BT4Bar6", "BOTTOMRIGHT", 61, -12)
	...
	if BT4Bar10 then
		...
		SUI_Bar6_BG:Show()
		SUI_Bar6_BG:SetPoint("BOTTOMLEFT", "BT4Bar10", "BOTTOMLEFT", -59, -11)
	...
	if BT4BarBagBar then
		...
		KeyRingButton:SetScale(0.78)
		MainMenuBarBackpackButton:SetScale(.82)
	end
...
-- TODO: remove all references to scale in Sui_ClassIcon calls since it is no longer used
	...
	local col, row = classIcons[class][1],classIcons[class][2];
...
function Sui_RaidIcon()
	...
	if (suiData.raidIcons ~= "Default") then
		iconFile = "Interface\\AddOns\\SpartanUI\\Media\\Icon_RaidTargets";
	end
	if (suiData.raidIcons == "Clean_Grey") then
		top = 0.5;
	end
	for i = 1, 10 do -- cycle through our 10 relevant unit frames
		local iconNum = GetRaidTargetIndex(UnitFrames[i].unit)
		local icon = _G[UnitFrames[i].frame.."_RaidIcon"]
...
function Sui_HealthMath(toon, barwidth, barheight, father)
	...
	UnitFrames[toonID].HealthPercent = healthPercent
end
-----------------------------------------------------------------------------------------------
...
function Sui_PowerMath(toon, barwidth, barheight, father)
	...
	if UnitPowerType(toon)==0 then
   	...
   	elseif UnitPowerType(toon)==3 then
   		...
   		texture4 = _G[father.."_FocusBar"]
   	elseif UnitPowerType(toon)==6 then
   		power = "Mana" -- Runic
   		texture2 = _G[father.."_RageBar"]
   		texture3 = _G[father.."_EnergyBar"]
   		texture4 = _G[father.."_FocusBar"]
	...
	-- Round the percentage to 0 decimals.
	local power = (UnitPower(toon) / UnitPowerMax(toon)) * 100
    ...
    count:SetText(UnitPower(toon))
    deficit:SetText(UnitPowerMax(toon) - UnitPower(toon))

    -- Only show the deficit if there is one
    if (UnitPowerMax(toon) == UnitPower(toon)) then
    	deficit:Hide()
    else
    	deficit:Show()
    end
	...
	local toonID = Sui_DeriveUnit(toon, "number", 10)
	if not toonID then return end
	UnitFrames[toonID].PowerPercent = powerPercent
end
...
function Sui_CastMath(toon, spell, rank, barwidth, barheight, texture)
	...
	if UnitFrames[toonID].isCasting==1 then
		spell, rank, spellName, _, startTime, endTime = UnitCastingInfo(toon)
		if not endTime then
			UnitFrames[toonID].isCasting=0
		else
			UnitFrames[toonID].spellDuration = (endTime - startTime)/1000
			UnitFrames[toonID].spellEnd = endTime/1000
		end
	end

	if UnitFrames[toonID].isChanneling==1 then
		spell, rank, spellName, _, startTime, endTime = UnitChannelInfo(toon)
		if not endTime then
			UnitFrames[toonID].isChanneling=0
		else
			UnitFrames[toonID].spellDuration = (endTime - startTime)/1000
			UnitFrames[toonID].spellEnd = endTime/1000
		end
	...
	if type(spell)=="string" then
		_G[UnitFrames[toonID].frame .. "_CastText"]:SetText(spell)
		_G[UnitFrames[toonID].frame .. "_CastText"]:Show()
		_G[UnitFrames[toonID].frame .. "_CastTime"]:SetText(UnitFrames[toonID].spellDuration)
		_G[UnitFrames[toonID].frame .. "_CastTime"]:Show()
	end
	if toonID==1 then
		_G[UnitFrames[toonID].frame .. "_CastLatency"]:SetText(SUI_lag)
		_G[UnitFrames[toonID].frame .. "_CastLatency"]:Show()
	end
	_G[UnitFrames[toonID].frame .. "_CastBar_Latency"]:SetAlpha(.5)

	if not UnitFrames[toonID].isCasting then
		UnitFrames[toonID].isCasting = 0
	end

	if not UnitFrames[toonID].isChanneling then
		UnitFrames[toonID].isChanneling = 0
	end

	if UnitFrames[toonID].isCasting==0 and UnitFrames[toonID].isChanneling==0 then
		Sui_Animation(UnitFrames[toonID].unit, "Cast", UnitFrames[toonID].frame .. "_CastBar", UnitFrames[toonID].castbar_width, UnitFrames[toonID].castbar_height, UnitFrames[toonID].direction, 1, toonID)
	end
end
...
function Sui_InitCastMath(toon, spell, rank, wildcard, value)
	...
	if not toonID then return end
	if (toonID<=6 or toonID==9) and (UnitFrames[toonID][wildcard]==1 and value==0) or value==1 then
		UnitFrames[toonID][wildcard] = value
		Sui_CastMath(toon, spell, rank, UnitFrames[toonID].castbar_width, UnitFrames[toonID].castbar_height, _G[UnitFrames[toonID].frame.."_CastBar"])
...
--|				toonID = corresponds to the UnitFrames[N] (fewer calls if simply passed)	|--
-----------------------------------------------------------------------------------------------
function Sui_Animation(toon, kind, texture, barwidth, barheight, direction, smoothdegree, toonID)
	...
	if kind=="Power" then
		local power
		if UnitPowerType(toon)==0 or UnitPowerType(toon)==6 then
   			power = "Mana"
   		elseif UnitPowerType(toon)==1 then
   			power = "Rage"
   		elseif UnitPowerType(toon)==2 then
   	...
   	elseif kind=="Cast" then
   		...
   		if not UnitFrames[toonID].spellEnd then return end
		local castPercent = 100 - ((UnitFrames[toonID].spellEnd - gameTime) / UnitFrames[toonID].spellDuration) * 100
		...
		UnitFrames[toonID].CastPercent = castPercent
		local castPoint = UnitFrames[toonID].spellEnd - gameTime		-- Time since cast started
		...
		local relativeLag = (SUI_lag/1000) / (UnitFrames[toonID].spellDuration)
		if not UnitFrames[toonID].frame then return end
		_G[UnitFrames[toonID].frame.."_CastTime"]:SetText(string.format("%.1f", castPrint))
		texture = _G[texture]

		if UnitFrames[toonID].isCasting==0 and UnitFrames[toonID].isChanneling==0 then
			UnitFrames[toonID][kind.."progress"] = 0
			UnitFrames[toonID][kind.."Percent"] = 0
			_G[UnitFrames[toonID].frame .. "_CastText"]:Hide()
			_G[UnitFrames[toonID].frame .. "_CastTime"]:Hide()
			_G[UnitFrames[toonID].frame .. "_CastLatency"]:Hide()
			_G[UnitFrames[toonID].frame .. "_CastBar_Latency"]:Hide()
			texture:Hide()
		end
	else
   		texture = _G[texture]
	...
	if not UnitFrames[toonID][kind.."progress"] then -- This variable is the stored value percent of a bars progress (1 - 100)
		UnitFrames[toonID][kind.."progress"] = 0
	elseif not string.find(UnitFrames[toonID][kind.."progress"], "%d+") then
		UnitFrames[toonID][kind.."progress"] = 0
	end
	local barProgress = UnitFrames[toonID][kind.."progress"]

	local barPercent = UnitFrames[toonID][kind.."Percent"]
	...
	-- Pass off globals
	UnitFrames[toonID][kind.."progress"] = barProgress
end
-----------------------------------------------------------------------------------------------
...
function Sui_XPBar()
	...
	if type(currentTime)=="number" then
		local sessionTime = GetTime() - Sui.SessionTime
		local timeSession = (maxXP-currentXP)/(Sui.SessionXP/sessionTime)
	...
	if suiChar.mobXP then
		SUI_XP_EstimatedMobs:SetText("Mobs to level: "..math.floor(((maxXP-currentXP)/suiChar.mobXP) + 0.5).." "..(suiChar.mobLast or "Quests"))
	end
	FrameAdvance = 0
end
-----------------------------------------------------------------------------------------------
...
function Sui_XPIncrease()
	...
	elseif FrameAdvance== 3 then
		SUI_XP_Increase:SetTexCoord( .50, .75, 0, 1)
	elseif FrameAdvance== 4 then
...
function Sui_Rep_TooltipVisibility(toggle)
	if toggle=="show" then
		SUI_Rep_Tooltip_Description:SetText(Sui_FactionDesc())
...
-----------------------------------------------------------------------------------------------
--|  Purpose:	Searches for which faction is currently being watched.						|--
-----------------------------------------------------------------------------------------------
function Sui_FactionDesc()
	if GetWatchedFactionInfo() then
		local numFactions = GetNumFactions()
		for i=1, numFactions do
			local _, description, _, _, _, _, _, _, _, _, _, isWatched, _ = GetFactionInfo(i)
			if isWatched then
				return description
			end
		end
		...
		if UnitExists(toon) and toonID<=10 then
			if toon=="target" then
				...
				elseif UnitClassification("target")=="rare" or UnitClassification("target")=="rareelite" then
					SUI_Target_EliteIcon:SetTexture("Interface\\AddOns\\SpartanUI\\Media\\Icon_Elite.blp")
			...
			Sui_HealthMath(toon, UnitFrames[toonID].healthbar_width, UnitFrames[toonID].healthbar_height, frame)
			Sui_DelayedCall("Sui_Unit_UpdateAFK_Subtask", toon..","..toonID, 1)
			if suiData.buffToggle then
				Sui_BuffScan(toon)
			end
			if toon~="focus" then
				Sui_PowerMath(toon, UnitFrames[toonID].powerbar_width, UnitFrames[toonID].powerbar_height, frame)
			end
...
function Sui_DeriveUnit(toon, desiredType, limit)
		...
		if start then
			for v = 1, 40, 1 do
				while i and i<=(limit or 10) do
					if UnitIsUnit(toon, UnitFrames[i].unit) then
			...
			while UnitFrames[i] and i<=(limit or #UnitFrames) do
				if toon==UnitFrames[i].unit then
					toonID, varType = i, "number"
					i = nil
				else
					i = i + 1
	...
	elseif varType == "number" then
		while UnitFrames[i] and i<=(limit or #UnitFrames) do
			if toon==i then
				toonID, varType = UnitFrames[i].unit, type(UnitFrames[i].unit)
				i = nil
			else
				i = i + 1
			end
	...
	if toonID and varType==desiredType then
		if desiredType=="number" and UnitFrames[toonID] and UnitFrames[toonID].frame then
			return toonID
...
------------------------------------------------------------------------------------------------
--|  Purpose:	Creates all buff frames for population based on UnitFrames[i].frame        |--
--|  Variables:	subject = "Buff", "Debuff" or "Weapon"										 |--
--|				i = current unit (1-8)														 |--
--|				ii = current buff (as many as there are)									 |--
--|				frame = from UnitFrames[i].frame, the name of the unit frame.				 |--
------------------------------------------------------------------------------------------------
function Sui_BuffCreateFrames()
	local i = 1
	while UnitFrames[i] and i<=10 do
		local state = "HELPFUL"
		local ii = 1
		while ii do
			...
			local direction = UnitFrames[i].direction
			local runDirection = UnitFrames[i].runDirection
			local father_string = UnitFrames[i].frame .. "_" .. subject .. "_" .. ii
			local master_string = UnitFrames[i].frame .. "_" .. subject .. "s"
			local icon, count, master, bar, timer
			local buffSize = UnitFrames[i].buffSize
			local scaleFactor = 1
			local buffPadding = UnitFrames[i].buffPadding
			...
			if not _G[master_string] then
				...
				if direction=="left" then
					if UnitFrames[i].unit ~= "player" then
						-- Party Buffs
						selfAnchor = "TOPLEFT"
				...
				elseif direction=="right" then
					if UnitFrames[i].unit ~= "target" then
				...
				master = CreateFrame("Frame", master_string, _G[UnitFrames[i].frame .. "Frame_Info"])
				master:SetWidth(buffSize)
				master:SetHeight(buffSize)
				master.bgTexture = master:CreateTexture(master_string.."_BuffBG")
				master.bgTexture:SetTexture("Interface\\AddOns\\SpartanUI\\Media\\Object_Buff_BG.blp")
				master.direction = UnitFrames[i].direction
				master.bgTexture:SetParent(master)
				master.bgTexture:SetAlpha(.55)

				master:SetPoint(selfAnchor, _G[UnitFrames[i].frame .. "_Button"], parentAnchor, xOffset, yOffset)

				if subject=="Debuff" then
					local debuffParent, multiplier = _G[UnitFrames[i].frame .. "_" .. "Buffs"], 1
					xOffset, yOffset = 0, (6 + buffPadding)
					if direction~=runDirection then
						parentAnchor = parentAnchorDebuff
						debuffParent = _G[UnitFrames[i].frame .. "_" .. "Buff_" .. UnitFrames[i].buffLimit]
				...
				elseif subject=="Weapon" then
					master:SetPoint(selfAnchor, _G[UnitFrames[i].frame .. "_" .. "Debuffs"], parentAnchor, 0, (6 + buffPadding))
				end
				master:SetParent(_G[UnitFrames[i].frame .. "Frame_Info"])
			else
			...
			if _G[UnitFrames[i].frame .. "_Buff_" .. (ii-1)] then
				if runDirection=="left" then
					father:SetPoint("BOTTOMRIGHT", _G[UnitFrames[i].frame .. "_".. subject .. "_" .. (ii-1)], "BOTTOMLEFT", -2, 0)
				elseif runDirection=="right" then
					father:SetPoint("BOTTOMLEFT", _G[UnitFrames[i].frame .. "_" .. subject .. "_" .. (ii-1)], "BOTTOMRIGHT", 2, 0)
				end
			else
				if runDirection=="left" then
			...
			if i=="1" then
				...
				bar:SetTexture("Interface\\AddOns\\SpartanUI\\Media\\Object_Bar_SpartanBlue.blp")
			...
			ii = ii + 1
			if state=="HELPFUL" and ii > UnitFrames[i].buffLimit then
				state = "HARMFUL"
				ii = 1
			elseif state=="HARMFUL" and ii > UnitFrames[i].debuffLimit and UnitFrames[i].unit=="player" then
				state = "WEAPON"
				ii = 1
			elseif state=="HARMFUL" and ii > UnitFrames[i].debuffLimit then
				ii = nil
			elseif state=="WEAPON" and ii > UnitFrames[i].weaponLimit then
				ii = nil
			end
		end
...
function Sui_BuffScan(toon)
	...
	UnitFrames[toonID].Buff = {}
	UnitFrames[toonID].Debuff = {}
	local terms = {
		"name",
		"rank",
		"icon",
	...
	local buffValues, debuffValues = {}, {}
	for i=1, 16, 1 do
		if toon=="player" then
			buffValues = { UnitAura(toon, i) }
		else
			buffValues = { UnitBuff(toon, i) }
		end
		debuffValues = { UnitDebuff(toon, i) }
		for k=1, 9, 1 do
			if buffValues[k] then
				if not UnitFrames[toonID].Buff[i] then
					UnitFrames[toonID].Buff[i] = {}
				end
				UnitFrames[toonID].Buff[i][terms[k]] =  buffValues[k]
			end
			if debuffValues[k] then
				if not UnitFrames[toonID].Debuff[i] then
					UnitFrames[toonID].Debuff[i] = {}
				end
				UnitFrames[toonID].Debuff[i][terms[k]] =  debuffValues[k]
			end
		end
	end
...
--|  Arguments:	toon = "player", "party1", etc...											|--
--|  Variables: i = curent unit (a.k.a. toon) found in UnitFrames[x].unit					|--
...
function Sui_BuffPopulate(toon)
	...
	if i<=10 and i~=8 then
		...
		while ii do
			...
			local father = _G[UnitFrames[i].frame .. "_" .. subject .. "_" .. ii]
			local icon = _G[UnitFrames[i].frame .. "_" .. subject .. "_" .. ii .. "_Icon"]
			local count = _G[UnitFrames[i].frame .. "_" .. subject .. "_" .. ii .. "_Count"]

			local currentBuff
			if UnitFrames[i][subject] then
				if UnitFrames[i][subject][ii] then
					father:Show()
					icon:SetTexture(UnitFrames[i][subject][ii].icon)

					-- Fix for zeros and ones
					local counter
					if UnitFrames[i][subject][ii].count == 0 or UnitFrames[i][subject][ii].count == 1 then
						counter = nil
					else
						counter = UnitFrames[i][subject][ii].count
					end
					count:SetText(counter)
					father.state = subject
					father.index = ii
					father.castable = UnitFrames[i][subject].isStealable
					father.direction = UnitFrames[i].direction
					father.toon = UnitFrames[i].unit
					currentBuff = true
				else
					currentBuff = false
				end
			...
			local nextBuff
			if UnitFrames[i][subject] then
				if UnitFrames[i][subject][ii + 1] then
					nextBuff = true
				else
					nextBuff = false
				end
			...
			if toon=="player" then
				if state=="HELPFUL" and (nextBuff==false or ii > UnitFrames[i].buffLimit) then
					Sui_BuffCleanup(UnitFrames[i].frame .. "_" .. subject, ii, nextBuff, currentBuff, i)
					ii = 1
				elseif state=="HARMFUL" and (nextBuff==false or ii > UnitFrames[i].debuffLimit) then
					Sui_BuffCleanup(UnitFrames[i].frame .. "_" .. subject, ii, nextBuff, currentBuff, i)
					ii = 1
				elseif state=="WEAPON" and (nextBuff==false or ii > UnitFrames[i].weaponLimit) then
					Sui_BuffCleanup(UnitFrames[i].frame .. "_" .. subject, ii, nextBuff, currentBuff, i)
					ii = nil
				end
			else
...
function Sui_BuffUpdate()
	...
	while x do
		...
		if UnitFrames[1][subject] then
			if UnitFrames[1][subject][x] then
				if UnitFrames[1][subject][x].buffTime then
				-- end DB test
					local start = UnitFrames[1][subject][x].buffTime
					local current = (GetTime() - UnitFrames[1][subject][x].buffTimeMarker)
					...
					local bar = _G[UnitFrames[1].frame .. "_" .. subject .. "_" .. x .. "_Bar"]
					local timer = _G[UnitFrames[1].frame .. "_" .. subject .. "_" .. x .. "_Timer"]
					local buffSize = (_G[UnitFrames[1].frame .. "_" .. subject .. "_" .. x .. "_Icon"]).buffSize
...
--|				i = curent unit (a.k.a. toon) found in UnitFrames.x						|--
-----------------------------------------------------------------------------------------------
function Sui_BuffCleanup(frame, ii, next, current, i)
	...
	if _G[frame .. "s"] then
		if _G[frame .. "s"].bgTexture then
			...
			local padding = UnitFrames[i].buffPadding
    	    ...
    	    if start then
    		   ...
    		   -- DEFAULT_CHAT_FRAME:AddMessage("father: "..frame_string.." | index: "..index.." | state: "..state.." | direction: "..direction.." | toon: "..toon, 0.5, 0.5, 0.5)
    	    	...
    	    	GameTooltip:SetOwner(father, direction)
   				if toon=="player" and state=="Weapon" then
					GameTooltip:SetInventoryItem("player", index)
				end
				GameTooltip:SetUnitAura(toon, index)
    	    	GameTooltip:Show()
    	...
    	if frame:IsVisible() and MouseIsOver(frame) then
    	    ...
    	    if start then
    	    	...
    	    	if father.toon == "player" then
	    	    	CancelUnitBuff("player", father.index)
	    	    end
    	    else
    	    	start, finish = string.find(frame_string, "_Weapon_", 1, true)
    	    	if start then
...
function Sui_BuffConjure()
	...
	while UnitFrames[i] and i <= 10 do
		UnitFrames[i].Buff, UnitFrames[i].Debuff, UnitFrames[i].Weapon = {}, {}, {}
		buffLimit, debuffLimit, weaponLimit = UnitFrames[i].buffLimit, UnitFrames[i].debuffLimit, UnitFrames[i].weaponLimit
		iB, iD, iW = 1, 1, 1
		while iB <= buffLimit do
			...
			table.insert(UnitFrames[i].Buff, tempDB)
			iB = iB + 1
		end
		while iD <= debuffLimit do
			...
			table.insert(UnitFrames[i].Debuff, tempDB)
			iD = iD + 1
		end
		if weaponLimit then
			while iW <= weaponLimit do
				...
				table.insert(UnitFrames[i].Weapon, tempDB)
				iW = iW + 1
			end
		end
		DEFAULT_CHAT_FRAME:AddMessage("Conjured buffs for "..UnitFrames[i].unit..".  "..(iB-1).." buffs, "..(iD-1).." debuffs, and "..(iW-1).." weapon buffs.", 0.5, 0.5, 0.5)
		Sui_BuffPopulate(UnitFrames[i].unit)
		i = i + 1
	end
end
-----------------------------------------------------------------------------------------------
...
function Sui_Unit_UpdateAFK_Subtask(toon, toonID)
	...
	if toonID<=6 then
		local icon = _G[UnitFrames[toonID].frame .. "_ReadyMaybe"]
		if UnitIsAFK(toon) then
			icon:SetTexture("Interface\\AddOns\\SpartanUI\\Media\\Icon_AFK.blp")
			icon:SetHeight(32)
			icon:Show()
		else
			icon:Hide()
			icon:SetHeight(70)
			icon:SetTexture("Interface\\AddOns\\SpartanUI\\Media\\Icon_Readycheck_Maybe.blp")
		end
	end
end
-----------------------------------------------------------------------------------------------
...
function Sui_DelayedCall(functionName, parameters, timeDelay)
	if not functionName then
		if Sui.delayedCalls and Sui.delayedCalls[1] then
			local i = 1
			while i and i<5 do
				if Sui.delayedCalls[i] and Sui.delayedCalls[i][3] <= GetTime() then
					_G[Sui.delayedCalls[i][1]](unpack(Sui.delayedCalls[i][2]))
					-- DEFAULT_CHAT_FRAME:AddMessage(Sui.delayedCalls[i][1].." called.", 0.5, 0.5, 0.5)
					table.remove(Sui.delayedCalls, i)
				elseif Sui.delayedCalls[(i+1)] then
					i = i + 1
				else
					i = nil
				end
		...
		if not Sui.delayedCalls then
			Sui.delayedCalls = {}
		end
		table.insert(Sui.delayedCalls, i)
	end
end
-----------------------------------------------------------------------------------------------
...
function Sui_SpinCam()
	...
	elseif toggle=="1" then
		if SpinCam==false then
			SetCVar("cameraYawMoveSpeed","7.33")
			MoveViewRightStart()
			SetView(5)
		elseif SpinCam==true then
			MoveViewRightStop()
...
function Sui_OnEvent()
		...
		if arg1=="SUI" then
			...
			if task=="find" then
				...
				elseif subject=="version" then
					SendAddonMessage("SUI", "deliver "..UnitName("player").." "..subject.." "..CurrentVersion, "WHISPER", caller)
			...
			elseif task=="deliver" then
				...
				elseif subject=="AFK" then
					local toonID
					for v=3, 6, 1 do
						if UnitIsUnit(UnitFrames[v].unit, caller) then
							toonID = v
							if value=="true" then
	...
	elseif event=="UNIT_LEVEL" then
		...
		if not toonID then return end
		if _G[UnitFrames[toonID].frame .. "_Level"] then
			_G[UnitFrames[toonID].frame .. "_Level"]:SetText(UnitLevel(arg1))
		end
		if arg1=="player" then
	...
	elseif event=="UNIT_PORTRAIT_UPDATE" then
		...
		if _G[UnitFrames[toonID].frame.."_Portrait"] then
			SetPortraitTexture(_G[UnitFrames[toonID].frame.."_Portrait"], arg1)
		end
		if arg1=="player" then
	...
	elseif event=="PLAYER_FLAGS_CHANGED" then
		...
		if not toonID then return end
		Sui_Unit_UpdateAFK_Subtask(UnitFrames[toonID].unit, toonID)
	elseif event=="PLAYER_FOCUS_CHANGED" then
		Sui_FocusUpdate()
	elseif event=="PLAYER_ENTERING_WORLD" then
		if RuneFrame then
			RuneFrame:ClearAllPoints()
			RuneFrame:SetPoint("BOTTOM", SpartanUI, "BOTTOM", 0, 170)
		end
		Sui_SetupMembers("all", "setup")
		Sui_XPBar()
		Sui_Unit_Update()
	...
	elseif event=="VOICE_START" or event=="VOICE_PUSH_TO_TALK_START" then
		...
		_G[UnitFrames[toonID].frame .. "_VoiceIcon"]:Show()
	elseif event=="VOICE_STOP" or event=="VOICE_PUSH_TO_TALK_STOP" then
		local toonID = Sui_DeriveUnit(arg1, "number", 6)
		if not toonID then toonID=1 end
		_G[UnitFrames[toonID].frame .. "_VoiceIcon"]:Hide()
	end
end
-----------------------------------------------------------------------------------------------
...
function Sui_ReadyCheck(toon, ready, timeout)
	...
	if toon=="finish" or not toon then
		for toonID = 1, 10, 1 do
			if _G[UnitFrames[toonID].frame.."_ReadyYes"] then
				_G[UnitFrames[toonID].frame.."_ReadyYes"]:Hide()
				_G[UnitFrames[toonID].frame.."_ReadyMaybe"]:Hide()
				_G[UnitFrames[toonID].frame.."_ReadyNo"]:Hide()
			end
		end
	-- READY_CHECK
	elseif timeout then
		...
		if UnitIsUnit(toon, "player") then
			...
			for toonID = 1, 10, 1 do
				if UnitIsUnit(toon, UnitFrames[toonID].unit) then
					_G[UnitFrames[toonID].frame.."_ReadyYes"]:Show()
					_G[UnitFrames[toonID].frame.."_ReadyMaybe"]:Hide()
				elseif _G[UnitFrames[toonID].frame.."_ReadyMaybe"] then
					_G[UnitFrames[toonID].frame.."_ReadyMaybe"]:Show()
				end
			end
		end
	-- READY_CHECK_CONFIRM
	elseif ready then
		if UnitIsUnit(suiData.readyCheck, "player") then
			...
			if ready==1 then
				...
				_G[UnitFrames[toonID].frame.."_ReadyMaybe"]:Hide()
			elseif ready==0 then
				_G[UnitFrames[toonID].frame.."_ReadyNo"]:Show()
				_G[UnitFrames[toonID].frame.."_ReadyMaybe"]:Hide()
			end
		end
	end
...
function Sui_OnUpdate(self, elapsed)
	if suiData then
		GeneralInterval = GeneralInterval + elapsed
		FrequentInterval = FrequentInterval + elapsed
		AnimationInterval = AnimationInterval + elapsed
		FastInterval = FastInterval + elapsed
		if (GeneralInterval > UpdateInterval) then				-- 1FPS
			GeneralInterval = 0
		...
		if (FrequentInterval > UpdateInterval*0.36) then		-- around 3FPS
			FrequentInterval = 0

			if UnitExists("targettarget") then
				if not UnitFrames[7].unitGUID then
					UnitFrames[7].unitGUID = UnitGUID("targettarget")
					Sui_SetupMembers("targettarget", "setup")
				end
				if UnitFrames[7].unitGUID~=UnitGUID("targettarget") then
					UnitFrames[7].unitGUID = UnitGUID("targettarget")
					Sui_SetupMembers("targettarget", "setup")
				end
			end
		...
		if (AnimationInterval > (UpdateInterval*0.08)) then		-- 12.5 FPS
			AnimationInterval = 0
			FrameAdvance = FrameAdvance + 1

			local i = 1
			while i <= 10 do
				if i~=8 and _G[UnitFrames[i].frame]:IsVisible() then
					Sui_Animation(UnitFrames[i].unit, "Health", UnitFrames[i].frame .. "_HealthBar", UnitFrames[i].healthbar_width, UnitFrames[i].healthbar_height, UnitFrames[i].direction, 3, i)
					if i~=9 then
						Sui_Animation(UnitFrames[i].unit, "Power", UnitFrames[i].frame, UnitFrames[i].powerbar_width, UnitFrames[i].powerbar_height, UnitFrames[i].direction, 3, i)
					end
				end
				i = i + 1
			end

			if (FrameAdvance < 5) then
				SUI_XP_Increase:Show()
				Sui_XPIncrease()
			elseif (FrameAdvance > 5) then
		...
		--|  CastBar animation  |--
		if (FastInterval > (UpdateInterval/32)) then				-- 32 FPS
			...
			local i = 1
			while i <= 10 do
				if UnitFrames[i].isCasting==1 or UnitFrames[i].isChanneling==1 then
					Sui_Animation(UnitFrames[i].unit, "Cast", UnitFrames[i].frame .. "_CastBar", UnitFrames[i].castbar_width, UnitFrames[i].castbar_height, UnitFrames[i].direction, 1, i)
				end
				i = i + 1
			end
